...
mod ecdh_p384_hkdf {
    ...
    pub fn print_shared_keys() {
        ...
        // Misuse of PRK, just for correctness checking
        let alice_shared_with_bob_hk_4 = Hkdf::<Sha3_384>::from_prk(alice_shared_with_bob.raw_secret_bytes().as_slice())
            .expect("PRK should be conform the size of hash, i.e. PRK should be large enough and equal to used hash function output");
        let mut alice_shared_with_bob_hk_bytes_4 = [0u8; 48];
        let _ = alice_shared_with_bob_hk_4.expand(&[0u8; 0], &mut alice_shared_with_bob_hk_bytes_4)
            .expect("48 bytes + info expand bytes is a valid length for Sha3_384 hash with expanding operation to output");
